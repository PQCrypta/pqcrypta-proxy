...
pub struct ServerConfig {
    ...
    /// Encrypted Client Hello (ECH, draft-ietf-tls-esni) configurations this
    /// server is willing to accept. `None` (the default) means ECH is
    /// disabled - incoming `encrypted_client_hello` extensions are ignored
    /// and the handshake proceeds using the (unencrypted) ClientHelloOuter
    /// as normal, exactly as if this field didn't exist. Set via
    /// [`ConfigBuilder::with_ech`](crate::ConfigBuilder::with_ech) (for
    /// `ConfigBuilder<ServerConfig, _>`) or directly on an existing config.
    pub ech: Option<Arc<super::ech::ServerEchConfigs>>,
