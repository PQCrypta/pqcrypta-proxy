...
mod client_hello {
    ...
    fn emit_server_hello(
        ...
        // Do key exchange. Performed ahead of sending ServerHello (unlike
        // upstream rustls) because the ECH acceptance confirmation below
        // needs a `KeyScheduleHandshakeStart` to reach its HKDF provider -
        // the confirmation derivation does not itself depend on this (EC)DHE
        // output (see `KeyScheduleHandshakeStart::server_ech_confirmation_secret`),
        // so moving it earlier is safe.
        let mut key_schedule = key_schedule_pre_handshake.into_handshake(ckx.secret);

        // If this connection accepted an ECH offer (`resolve_ech` in
        // `server::hs`, run while still processing ClientHello), splice the
        // spec-mandated acceptance confirmation (draft-ietf-tls-esni-25
        // section 7.2) into the low 8 bytes of ServerHello.random. rustls's
        // own client-side ECH (`client::ech::EchState::confirm_acceptance`)
        // requires this exact value to recognize acceptance - without it, a
        // real ECH client would still conclude ECH was rejected (even though
        // decryption succeeded here) and fall back to validating the outer
        // ClientHelloOuter/public_name identity.
        if let crate::server::ech::EchStatus::Accepted {
            client_hello_inner_random,
        } = cx.data.ech_status
        {
            let MessagePayload::Handshake { encoded, parsed } = &mut sh.payload else {
                unreachable!("sh is always constructed as a Handshake message above");
            };
            let mut zeroed_encoding = encoded.bytes().to_vec();
            zeroed_encoding[crate::msgs::handshake::SERVER_HELLO_ECH_CONFIRMATION_SPAN]
                .fill(0x00);
            let confirmation_hash = transcript.hash_given(&zeroed_encoding);
            let confirmation = key_schedule.server_ech_confirmation_secret(
                &client_hello_inner_random,
                confirmation_hash,
            );

            let HandshakePayload::ServerHello(server_hello) = &mut parsed.0 else {
                unreachable!("sh is always constructed as a ServerHello message above");
            };
            server_hello.random.0[24..].copy_from_slice(&confirmation);

            let mut re_encoded = Vec::new();
            parsed.payload_encode(&mut re_encoded, crate::msgs::handshake::Encoding::Standard);
            *encoded = Payload::Owned(re_encoded);
        }
    ...
    fn emit_encrypted_extensions(
        ...
        // If the client offered ECH but we couldn't decrypt it (wrong/stale
        // config_id, or a GREASE offer with no real config on the client's
        // side), advertise our current configs so a real ECH client can
        // retry correctly, per draft-ietf-tls-esni-25 section 7.1.1.
        if let (crate::server::ech::EchStatus::Rejected, Some(ech_configs)) =
            (cx.data.ech_status, &config.ech)
        {
            ep.extensions.encrypted_client_hello_ack = Some(
                crate::msgs::handshake::ServerEncryptedClientHello {
