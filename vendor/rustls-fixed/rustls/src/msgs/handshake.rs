...
/// The byte range, within an encoded ServerHello handshake message, of the last
/// 8 bytes of the `random` field - used both to compute (server side) and
/// verify (client side) the ECH acceptance confirmation signal of
/// draft-ietf-tls-esni-25 section 7.2. Has:
/// - a HandshakeType (1 byte),
/// - an exterior length (3 bytes),
/// - the legacy_version (2 bytes), and
/// - the balance of the random field (24 bytes).
pub(crate) const SERVER_HELLO_ECH_CONFIRMATION_SPAN: core::ops::Range<usize> =
    (1 + 3 + 2 + 24)..(1 + 3 + 2 + 32);
