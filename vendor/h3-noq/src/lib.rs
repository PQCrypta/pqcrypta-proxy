...
//!
//! This module implements the `h3::quic` traits over
//! [noq](https://github.com/n0-computer/noq), a multipath-capable fork of
//! quinn. It is a direct port of the upstream `h3-quinn` 0.0.10 crate, with
//! three noq-specific adjustments where noq's stream API diverges from
//! quinn's (documented inline):
//!   - noq's ordered `RecvStream::read_chunk(max_len)` returns `Bytes`
//!     directly (no `ordered: bool` argument, no `Chunk` wrapper).
//!   - noq's `ReadError` has no `IllegalOrderedRead` variant.
//! Everything else is a mechanical `quinn::` -> `noq::` type swap; the two
//! crates share identical names/shapes for `Connection`, `SendStream`,
//! `RecvStream`, `OpenBi`/`OpenUni`/`AcceptBi`/`AcceptUni`, `VarInt`,
//! `ConnectionError`, and `WriteError`.
#![deny(missing_docs)]
    ...
    fn poll_data(
        ...
        if let Some(mut stream) = self.stream.take() {
            self.read_chunk_fut.set(async move {
                // noq: ordered read; returns Option<Bytes> directly.
...
fn convert_read_error_to_stream_error(error: ReadError) -> StreamErrorIncoming {
    match error {
        ...
        // noq has no `IllegalOrderedRead` variant (we only ever do ordered
        // reads here anyway).
