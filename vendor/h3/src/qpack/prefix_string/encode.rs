...
impl HuffmanEncoder {
    ...
    /// Start with the output already at its final length.
    ///
    /// `ensure_free_space` otherwise grows the buffer a byte at a time, pushing
    /// 255 as filler, reallocating as it goes. The encoded length is known up
    /// front from the code table, so it can simply be allocated once. 255 is the
    /// same filler, and it is what `write_bits` requires: it asserts the bits it
    /// is about to write are currently 1.
    fn with_len(len: usize) -> HuffmanEncoder {
        HuffmanEncoder {
            buffer_pos: BitWindow::new(),
            buffer: vec![255u8; len],
        }
    }
